...
(function () {
	...
	ops[opname]                  = typedArrayFunction("function (a, b, c)    {            a = b " + op + " c; }  ");
	ops[opname + "_mask"]        = typedArrayFunction("function (a, b, c, m) { if ( m ) { a = b " + op + " c; } }");
	ops[opname + "eq"]           = typedArrayFunction("function (a, b   )    {            a " + op + "= b;    }  ");
	ops[opname + "eq" + "_mask"] = typedArrayFunction("function (a, b   , m) { if ( m ) { a " + op + "= b;    } }");
    ...
    ops.any  = typedArrayFunction(function (a) { if ( a ) { return true;  } });
    ops.all  = typedArrayFunction(function (a) { if (!a ) { return false; } });
    ops.sum  = typedArrayFunction(function (a) {
	var sum = 0; 
	// ----
	    sum += a;
	// ----
    ...
    ops.prod = typedArrayFunction(function (a) {
	var prd = 0;
	// ----
	    prd *= a;
	// ----
    ...
    ops.inf  = typedArrayFunction(function (a) {
	var inf =  Infinity;
	// ----
	    if ( a < inf ) { inf = a; }
	// ----
    ...
    ops.sup  = typedArrayFunction(function (a) {
	var sup = -Infinity;
	// ----
	    if ( a > sup ) { sup = a; }
	// ----
    ...
    ops.assign = typedArrayFunction(function (a, b) { a = b; });
    ops.equils = typedArrayFunction(function (a, b) { if ( a !== b ) { return false; } });
    ops.random = typedArrayFunction(function (a)    { a = Math.random(); });
}());
